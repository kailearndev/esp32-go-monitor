...
lv_obj_t *tech_badge, *datetime_label;
...
void smooth_look(int tx, int ty, int time_ms)
{
    ...
    lv_anim_set_var(&a, eye_l);
    lv_anim_set_values(&a, lv_obj_get_style_translate_x(eye_l, 0), tx);
    lv_anim_start(&a);
    lv_anim_set_var(&a, eye_r);
    lv_anim_set_values(&a, lv_obj_get_style_translate_x(eye_r, 0), tx);
    lv_anim_start(&a);
    lv_anim_set_exec_cb(&a, anim_y_cb);
    lv_anim_set_var(&a, eye_l);
    lv_anim_set_values(&a, lv_obj_get_style_translate_y(eye_l, 0), ty);
    lv_anim_start(&a);
    lv_anim_set_var(&a, eye_r);
    lv_anim_set_values(&a, lv_obj_get_style_translate_y(eye_r, 0), ty);
    lv_anim_start(&a);
}
...
void smooth_size(int w, int h, int radius, int time_ms)
{
    ...
    lv_anim_set_var(&a, eye_l);
    lv_anim_set_values(&a, lv_obj_get_width(eye_l), w);
    lv_anim_start(&a);
    lv_anim_set_var(&a, eye_r);
    lv_anim_set_values(&a, lv_obj_get_width(eye_r), w);
    lv_anim_start(&a);
    lv_anim_set_exec_cb(&a, anim_h_cb);
    lv_anim_set_var(&a, eye_l);
    lv_anim_set_values(&a, lv_obj_get_height(eye_l), h);
    lv_anim_start(&a);
    lv_anim_set_var(&a, eye_r);
    lv_anim_set_values(&a, lv_obj_get_height(eye_r), h);
    lv_anim_start(&a);
    lv_obj_set_style_radius(eye_l, radius, 0);
    lv_obj_set_style_radius(eye_r, radius, 0);
}
...
void blink_eye()
{
    ...
    lv_anim_set_exec_cb(&a, anim_h_cb);
    lv_anim_set_var(&a, eye_l);
    lv_anim_set_values(&a, lv_obj_get_height(eye_l), 4);
    lv_anim_start(&a);
    lv_anim_set_var(&a, eye_r);
    lv_anim_set_values(&a, lv_obj_get_height(eye_r), 4);
    lv_anim_start(&a);
}
...
void squint_eye()
{
    ...
    lv_anim_set_exec_cb(&a, anim_h_cb);
    lv_anim_set_var(&a, eye_l);
    lv_anim_set_values(&a, lv_obj_get_height(eye_l), 18);
    lv_anim_start(&a);
    lv_anim_set_var(&a, eye_r);
    lv_anim_set_values(&a, lv_obj_get_height(eye_r), 18);
    lv_anim_start(&a);
}
...
void yawn_anim()
{
    ...
    lv_anim_set_exec_cb(&a, anim_h_cb);
    lv_anim_set_var(&a, eye_l);
    lv_anim_set_values(&a, 60, 10);
    lv_anim_start(&a);
    lv_anim_set_var(&a, eye_r);
    lv_anim_set_values(&a, 60, 10);
    lv_anim_start(&a);
    lv_timer_t *timer = lv_timer_create([](lv_timer_t *t)
                                        { isYawning = false; }, 1500, NULL);
...
void process_sensors_and_animations()
{
    ...
    if (isPanicking)
    {
        ...
        if (millis() - lastJitter > 60)
        {
            smooth_look(random(-8, 9), random(-8, 9), 50);
            lastJitter = millis();
        }
    }
    else
    {
        if (current_state == "IDLE" && !isYawning && (millis() - lastActivityTime > 30000))
        {
            yawn_anim();
            lastActivityTime = millis();
        }
        ...
        if (millis() - lastLookTime > 3000)
        {
            ...
            {
                if (random(0, 2) == 0)
                    smooth_look(random(-30, 31), random(-10, 11), 400);
                else
                    smooth_look(0, 0, 400);
            }
            lastLookTime = millis();
...
void parseSerial(String data)
{
    ...
    if (p[0] > 0 && p[5] > p[4])
    {
        ...
        if (current_state != "IDLE" || current_cpu > 20)
            lastActivityTime = millis();
...
void setup()
{
    ...
    lv_disp_drv_init(&d);
    d.hor_res = screenWidth;
    d.ver_res = screenHeight;
